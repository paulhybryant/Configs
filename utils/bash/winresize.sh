...
if [[ $WIN_X -ge 1366 ]]; then
  if [[ $1 == "LEFT" ]]; then
    log_run 'wmctrl -r :ACTIVE: -e 0,1366,0,960,1100'
  elif [[ $1 == "RIGHT" ]]; then
    log_run 'wmctrl -r :ACTIVE: -e 0,2326,0,960,1100'
  else
    zenity --info --text="Unknown parameter $1"
  fi
else
  if [[ $1 == "LEFT" ]]; then
    log_run 'wmctrl -r :ACTIVE: -e 0,0,470,683,716'
  elif [[ $1 == "RIGHT" ]]; then
    log_run 'wmctrl -r :ACTIVE: -e 0,683,470,683,716'
  else
    zenity --info --text="Unknown parameter $1"
  fi
fi
